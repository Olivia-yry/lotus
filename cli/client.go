	...
	Action: func(cctx *cli.Context) error {
		...
		if watch {
			...
			for {
				...
				err = outputStorageDeals(ctx, api, localDeals, verbose, color)
				if err != nil {
					return err
				}
				...
				case updated := <-updates:
					...
					for i, existing := range localDeals {
						if existing.ProposalCid.Equals(updated.ProposalCid) &&
							existing.CreationTime.Equal(updated.CreationTime) {
		...
		return outputStorageDeals(ctx, api, localDeals, cctx.Bool("verbose"), cctx.Bool("color"))
	},
}
...
func outputStorageDeals(ctx context.Context, full api.FullNode, localDeals []api.DealInfo, verbose bool, color bool) error {
